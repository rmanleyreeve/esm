...
public class ApproveEntrypointCommentForm {
	...
	EntrypointCommentsTable.Row sRow;
	...
	public static void main (String [] args) {
		...
		try {
			ApproveEntrypointCommentForm aecf = new ApproveEntrypointCommentForm(2);
			aecf.complete();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public ApproveEntrypointCommentForm(int _commentID) {
		me = this;
	...
	public boolean complete() {
		...
		ApproveEntrypointCommentForm.myshell = shlVideotelEsm;
		...
		lblTitle.setText("APPROVE ENTRYPOINT COMMENT");
		...
		try {
			sRow = EntrypointCommentsTable.getRow(commentID);
		...
		lblSComment.setText("Entrypoint\nComment:");		
		...
		ok.addSelectionListener (new SelectionAdapter () {
			...
			public void widgetSelected (SelectionEvent e) {
				try {
					...
					sRow.update();
					LogController.log("Entrypoint comment aproved");		        
					formOK = true;
				} catch (Exception e1) {
					e1.printStackTrace();
				}					
		...
		LogController.log("Approve Entrypoint Comment form closed");	
