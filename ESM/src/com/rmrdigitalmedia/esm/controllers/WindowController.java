...
public class WindowController {
	...
	static Composite formHolder, pageSpacesList, pageSpaceDetail,
			pageAdministration, pageSpaceAudit, pageEntryAudit;
	static Label pageTitle, onlineStatus;
	String displayName;
	public static Button btnAddSpace, btnDeleteSpace, btnAdmin,
			btnViewSpaceDetails;
	static Button btnSpacesList, btnAddEntry, btnEditEntry, btnDeleteEntry,
			btnEntryList;
	...
	public static void main(String[] args) {
		...
		try {
			WindowController window = new WindowController(
					EsmUsersTable.getRow("USERNAME", "admin"));
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public WindowController(EsmUsersTable.Row user) {
		...
		this.displayName = user.getRank() + " " + user.getForename() + " "
				+ user.getSurname();
	...
	public void open() {
		...
		shell.setSize(bounds.width - 200, bounds.height - 100); // almost fill
																// screen
	...
	protected void createContents() {
		...
		Button foo = new Button(titleBar, SWT.NONE); // dummy button to take
														// default
		...
		btnAddSpace.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				if (asf.complete()) {
					LogController
							.log("New Space & Entry Point saved in database");
					showSpacesList();
				}
			}
		});
		...
		try {
			txt = "Vessel: "
					+ (String) EsmApplication.appData.getField("VESSEL")
					+ "       ";
		} catch (Exception e1) {
		}
	...
	public static void showSpaceDetail(int spaceID) {
		...
		btnAdmin.setEnabled(true);
		LogController
				.log("Loading Space Detail page for user selection: Space ID "
						+ spaceID);
		SpaceDetailView.buildPage(pageSpaceDetail, spaceID);
		stackLayout.topControl = pageSpaceDetail;
		try {
			pageTitle.setText("Space " + spaceID + ": "
					+ SpacesTable.getRow("ID", "" + spaceID).getName());
		} catch (SQLException e) {
			LogController.logEvent(me, C.WARNING, e);
		}
		formHolder.layout();
	...
	public static void showSpaceAuditChecklist(int spaceID) {
		...
		btnSpacesList.setVisible(false);
		btnAdmin.setEnabled(true);
		LogController.log("Displaying Internal Space Audit Checklist for ID:"
				+ spaceID);
	...
	public static void showSpaceAuditClassification(int spaceID) {
		...
		btnAdmin.setEnabled(true);
		LogController
				.log("Displaying Internal Space Audit Classification for ID:"
						+ spaceID);
	...
	public static void showEntryAuditChecklist(int entryID) {
		...
		btnSpacesList.setVisible(false);
		btnAdmin.setEnabled(true);
		LogController.log("Displaying Entry Point Audit Checklist for ID:"
				+ entryID);
	...
	public static void showEntryAuditClassification(int entryID) {
		...
		btnSpacesList.setVisible(false);
		btnAdmin.setEnabled(true);
		LogController.log("Displaying Entry Point Audit Classification for ID:"
				+ entryID);
	...
	public static void checkSpaceAlert(int spaceID) {
		...
		showAlert = (EsmApplication.appData.getField("SPACE_STATUS_" + spaceID)
				.equals("red"));
		if (showAlert) {
	...
	public static void showPhotoViewer(int spaceID, String fullPath,
			String thumbPath) {
