...
public class AuditController {
	...
	public static void calculateSpaceChecklistCompletion(int spaceID) {
		...
		if(row != null) {
			...
			String q2 = row.getQ2Boolean(); if( isN(q2) || (isY(q2) && C.notNullOrEmpty(row.getQ2Desc()))) { score +=1; }
			if(row.getQ3Boolean()!=null) { score +=1; }
			String q4 = row.getQ4Boolean(); if ( isN(q4) || ( isY(q4) && C.notNullOrEmpty(row.getQ4DimsH()) && C.notNullOrEmpty(row.getQ4DimsW()) ))  { score +=1; }
			if(row.getQ5Boolean()!=null) { score +=1; }
			if(row.getQ6Boolean()!=null) { score +=1; }
			String q7 = row.getQ7Boolean(); if (isN(q7) || ( isY(q7) && row.getQ7Rating()!=0 )) { score +=1; }
			String q8 = row.getQ8Boolean(); if (isN(q8) || ( isY(q8) && row.getQ8Rating()!=0 )) { score +=1; }
			String q9 = row.getQ9Boolean(); if (isN(q9) || ( isY(q9) && row.getQ9Rating()!=0 )) { score +=1; }
			String q10 = row.getQ10Boolean(); if (isN(q10) || ( isY(q10) && row.getQ10Rating()!=0 )) { score +=1; }
		...
		//System.out.println("Calculating space "+spaceID+" checklist: " + score + "/" + max + "=" + percent + "% -> progress=" + progress );
		//System.out.println("Elapsed: "+ (System.currentTimeMillis() - startTime));
	}
	...
	public static void calculateEntryChecklistCompletion(int entryID) {
		...
		if(row != null) {
			String q1 = row.getQ1Value(); if( row.getQ1Value()!=null ) { score +=1; }
			if( q1!=null && q1.equals("OUTSIDE") && row.getQ2Boolean()!=null) { score +=1; }
			if(row.getQ3Boolean()!=null) { score +=1; }
			...
			if( q7!=null ) { score +=1; }
			if( q7!=null && q7.equals("VERTICAL") && row.getQ8Boolean()!=null) { score +=1; }
			if( q7!=null && q7.equals("VERTICAL") && row.getQ9Boolean()!=null) { score +=1; }
