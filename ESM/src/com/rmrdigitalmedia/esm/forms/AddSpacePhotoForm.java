...
public class AddSpacePhotoForm {
	...
	public boolean complete() {
		...
		AddSpacePhotoForm.myshell = shlVideotelEsm;
		shlVideotelEsm.setSize(500, 340);
		shlVideotelEsm.setText("Videotel ESM");
		shlVideotelEsm.setImages(new Image[] { C.getImage(C.APP_ICON_16),
		...
		imgSelected.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1));
		...
		GridData gd_title = new GridData(SWT.LEFT, SWT.CENTER, true, false, 2,
				1);
		...
		GridData gd_comment = new GridData(SWT.LEFT, SWT.CENTER, true, false,
				2, 1);
		...
		ok.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				Text[] fields = { p_title, p_comment, imgSelected };
				Validation.validateFields(fields);
				String path = "";
				if (Validation.validateFields(fields)) {
					try {
						path = UploadController
								.uploadSpaceImagePath(spaceID, new String[] {
										imgToUploadPath, imgToUploadName });
					} catch (Exception ex) {
					}
					if (!path.equals("")) {
						try {
							PhotoMetadataTable.Row pRow = PhotoMetadataTable
									.getRow();
							...
							pRow.setPath(path);
							pRow.setCreatedDate(new Timestamp(new Date()
									.getTime()));
							pRow.setUpdateDate(new Timestamp(new Date()
									.getTime()));
							if (EsmUsersTable.getRow("ID", "" + authorID)
									.getAccessLevel() >= 2) {
							...
							pRow.insert();
							LogController
									.log("Photo comment added to database.");
						} catch (Exception e1) {
							LogController.logEvent("Photo Comment upload",
									C.NOTICE, e1);
						}
						formOK = true;
					}
