...
public class EntryAuditClassificationView {
	...
	public static void buildPage(final Composite parent, final int entryID) {
		...
		try {
			aRow = EntrypointClassificationAuditTable.getRow("ENTRYPOINT_ID",
					"" + entryID);
		} catch (SQLException e1) {
			LogController.logEvent(EntryAuditClassificationView.class, C.ERROR,
					e1);
		}
		empty = (aRow == null);
		...
		try {
			qText.add(0, null);
			qHints.add(0, null);
			qRows = EntrypointClassificationQuestionsTable
	...
	static void saveAudit(int entryID) {
		...
		if (empty) {
			try {
				EntrypointClassificationAuditTable.Row r = EntrypointClassificationAuditTable
						.getRow();
				r.setEntrypointID(entryID);
				r.insert();
			} catch (SQLException e1) {
				LogController.logEvent(EntryAuditClassificationView.class,
						C.FATAL, "ERROR INSERT ENTRY CLASSIFICATION ROW", e1);
			}
		}
		EntrypointClassificationAuditTable.Row aRow = null;
		try {
			...
			LogController.logEvent(EntryAuditClassificationView.class, C.FATAL,
					"ERROR SELECT ENTRY CLASSIFICATION ROW", ex);
		}
		if (aRow != null) {
			try {
				// 1
				...
				// 3
				if (q3_radio1.getSelection()) {
					aRow.setQ3Value(1);
				} else if (q3_radio2.getSelection()) {
					aRow.setQ3Value(2);
				} else if (q3_radio3.getSelection()) {
					aRow.setQ3Value(3);
				} else {
					aRow.setQ3Value(0);
				}
